...
function boxCategory(len, str){
  const pattern = getPattern(str);
  const table = PAYOUTS[len].box;
  if(table[pattern]) return table[pattern];
  if(pattern === String(len)){
    return {name:'Straight (Boxed \u2014 identical digits)', odds: PAYOUTS[len].straight};
  }
  return null;
}
...
function evaluateTicket(t){
  ...
  if(t.playType==='straight'){
    ...
    if(baseWin){
      const cat = boxCategory(len, t.winning);
  ...
  if(t.playType==='straight'){
    ...
    if(fireballWin){ fireballCategory='Straight'; fireballOdds=PAYOUTS[len].straight; }
  } else {
    for(const c of dedupedCombos){
      if(multisetEqual(t.player, c)){
        const cat = boxCategory(len, c);
...
const PAYLINES_DATA = [
  {
    ...
    rows: [
      {type:'Straight', example:'12', range:'Any 2-digit number, 00 to 99, exact order.', odds:90, straight:true},
      {type:'2-Way Box', example:'12', range:'All digits different, any order.', odds:45}
    ]
  },
  {
    pick: 'Pick 3',
    ...
    rows: [
      {type:'Straight', example:'123', range:'Any 3-digit number, 000 to 999, exact order.', odds:900, straight:true},
      {type:'6-Way Box', example:'123', range:'All digits different, any order.', odds:150},
      {type:'3-Way Box', example:'112', range:'Exactly two digits the same, any order.', odds:300}
    ]
  },
  {
    pick: 'Pick 4',
    ...
    rows: [
      {type:'Straight', example:'1234', range:'Any 4-digit number, 0000 to 9999, exact order.', odds:9000, straight:true},
      {type:'24-Way Box', example:'1234', range:'All digits different, any order.', odds:375},
      {type:'12-Way Box', example:'1123', range:'Exactly two digits the same, any order.', odds:750},
      {type:'6-Way Box', example:'1122', range:'Two sets of two digits the same, any order.', odds:1500},
      {type:'4-Way Box', example:'1112', range:'Three digits the same, any order.', odds:2250}
    ]
  },
  {
    pick: 'Pick 5',
    ...
    rows: [
      {type:'Straight', example:'12345', range:'Any 5-digit number, 00000 to 99999, exact order.', odds:90000, straight:true},
      {type:'120-Way Box', example:'12345', range:'All digits different, any order.', odds:750},
      {type:'60-Way Box', example:'11234', range:'Exactly two digits the same, any order.', odds:1500},
      {type:'30-Way Box', example:'11223', range:'Two sets of two digits the same, any order.', odds:3000},
      {type:'20-Way Box', example:'11123', range:'Three digits the same, any order.', odds:4500},
      {type:'10-Way Box', example:'11222', range:'One pair and one set of three identical digits, any order.', odds:9000},
      {type:'5-Way Box', example:'11112', range:'Four digits the same, any order.', odds:18000}
    ]
  }
];
  ...
  container.innerHTML = PAYLINES_DATA.map(group => `
    <div class="paylines-group">
      ...
      <table class="paylines-table">
        <thead>
          <tr><th>Combination Type</th><th>Example</th><th>Rule</th><th>Payout</th></tr>
        </thead>
        <tbody>
          ${group.rows.map(row => `
            <tr class="${row.straight ? 'pl-straight' : ''}">
              ...
              <td>${row.range}</td>
              <td class="pl-odds">${row.odds.toLocaleString('en-US')} to 1</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
